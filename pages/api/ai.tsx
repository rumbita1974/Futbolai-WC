...
function safeParseJSON(content: string) {
  ...
  try {
    ...
    try {
      ...
      console.log('✅ JSON re-parsed after fixing');
      return reparsed;
    } catch (e) {
      console.error('❌ Still cannot parse JSON, using fallback');
...
async function searchYouTube(searchTerm: string) {
  ...
  try {
    ...
    console.log('🔍 Optimized YouTube query:', query);
    ...
    if (response.data.items?.length > 0) {
      ...
      console.log('✅ YouTube video found:', videoUrl);
      return videoUrl;
    } else {
...
function getPublicYouTubeFallback(searchTerm: string) {
  ...
  for (const [key, videoId] of Object.entries(publicVideos)) {
    if (term.includes(key)) {
      console.log('🔄 Using public fallback for:', key);
